...
func main() {
	...
	gimulImges[GimulGreenJa], _, err = ebitenutil.NewImageFromFile("./green_ja.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulGreenJang], _, err = ebitenutil.NewImageFromFile("./green_jang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulGreenSang], _, err = ebitenutil.NewImageFromFile("./green_sang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulGreenWang], _, err = ebitenutil.NewImageFromFile("./green_wang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulRedJa], _, err = ebitenutil.NewImageFromFile("./red_ja.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulRedJang], _, err = ebitenutil.NewImageFromFile("./red_jang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulRedSang], _, err = ebitenutil.NewImageFromFile("./red_sang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}

	gimulImges[GimulRedWang], _, err = ebitenutil.NewImageFromFile("./red_wang.png", ebiten.FilterDefault)
	if err != nil {
		log.Fatalf("read file error: %v", err)
	}
